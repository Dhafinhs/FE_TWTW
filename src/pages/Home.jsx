...
function Home() {
  ...
  useEffect(() => {
    axios.get("https://betwtw-production.up.railway.app/students").then((response) => {
      ...
      // Check if ALL students have reached 100%
      const allFinished = data.every((s) => s.twtw_count >= targetPerStudent);

      if (allFinished && !confettiFired) {
        confetti({
          particleCount: 500,
          spread: 120,
          origin: { y: 0.6 },
      ...
      >
        <div className="animate-fadeIn">
          <h1 className="text-4xl md:text-6xl font-bold mb-4 text-center">
            Selamat Datang di TWTW Tracker
          </h1>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {top10.map((student, index) => {
              const percent = Math.min(
                (student.twtw_count / targetPerStudent) * 100,
                100
              );
              return (
                <FadeInSection key={student.id}>
                  <Link
                    ...
                    className="card-animate bg-blue-900 bg-opacity-60 backdrop-blur-md text-white rounded-lg shadow-xl p-6 flex flex-col items-center hover:scale-105 transform transition-transform hover:bg-blue-800"
                  >
                    <span className="text-sm text-orange-300 mb-1 font-semibold">
                      #{index + 1}
                    </span>
      ...
      <FadeInSection>
        <section className="p-6 rounded-lg mx-4">
          ...
          <p className="text-center text-gray-300 mb-4">
            Klik nama anda jika ingin edit data anda.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
